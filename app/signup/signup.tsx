...
export default function SignUp() {
  ...
  async function usersignup() {
    ...
    setIsLoading(true);
    try {
      let response = await axios.post(API_ENDPOINTS.REGISTER, item, {
        headers: {
  ...
  const isFormValid = () => {
    return (
      nama && validateEmail(email) && password.length >= 8 && address && phone
    );
  };
  ...
  return (
    <div className="flex  w-full h-full justify-center items-center">
      <Card className="w-[850px]  bg-white">
        ...
        <CardContent>
          <Card>
            <CardContent className="flex flex-row w-full h-full p-2">
